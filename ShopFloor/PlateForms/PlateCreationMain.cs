...
namespace ShopFloor.PlateForms
{
    public partial class PlateCreationMain : Form
    {
        ...
        private string testConnectionString = "Data Source=ovesql02;User ID=testUser;Password=21TriedToDrink;database=Manufacturing;Connection Timeout=60;Persist Security Info=False";
        ...
        private void InitMethods() {
            ...
            TiffCalculatedGridView.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.DisplayedCells;
        ...
        private void TillTypeCombo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            TillAreaLabel.Text = Math.Round(tillTotalArea, 2).ToString();
            ImageableAreaLabel.Text = Math.Round(tillImageableArea, 2).ToString();

            RecalculateAfterTillChange();
        ...
        private void RecalculateAfterTillChange() {

            int currentTab = TiffEntryTabControl.SelectedIndex;
            int totalTabs = TiffEntryTabControl.TabCount;

            int tabRotater = currentTab;
            bool isRotationComplete = false;
            int maxIndex = totalTabs - 1;

            while (isRotationComplete == false) {

                TiffEntryTabControl.SelectedIndex = tabRotater;
                TiffSaveFunction();

                if (tabRotater < maxIndex)
                    tabRotater++;

                else
                    tabRotater = 0;

                if (tabRotater == currentTab)
                {
                    TiffEntryTabControl.SelectedIndex = tabRotater;
        ...
        private void FillCalculatedGrid()
        {
            ...
            PlateCreationTotal.TotalImageableScrap =Math.Round(tillImageableArea - PlateCreationTotal.TotalTiffImageableArea, 2);
            ...
            if (TotalsGridView.Rows.Count == 0)
            {
                TotalsGridView.Rows.Add(row);
            }

            row = (DataGridViewRow)TotalsGridView.Rows[0].Clone();
        ...
        private void TheFinalQuery(object sender, EventArgs e)
        {
            ...
            string nameOfCreator = StartupForm.UserName;
            Connection.ConnectionString = testConnectionString;
            if (finalize.TheFinalQuery(plateTiffInfoList, plateTillTiffCombo, plateTiffRowItemsList, Connection, nameOfCreator))
            {

...
namespace ShopFloor
{
	...
	/// </summary>
   public class FingerPrintAdmin : System.Windows.Forms.Form
   {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private SqlConnection connection1 = new SqlConnection("Data Source=ovesql01;User ID=sa;Password=c0l0mbIa;database=Inventory Control;Connection Timeout=60;Persist Security Info=False");
      ...
      static void Main() 
      {
         Application.Run(new FingerPrintAdmin());
      }
      ...
      private void BtnRegister_Click(object sender, System.EventArgs e)
      {
         ...
         if (iError == (Int32)SGFPMError.ERROR_NONE)
         {
                if (matched)
                {
                    ...
                    command = new SqlCommand("INSERT INTO [User Fingerprint Table] VALUES ('pkellyi', @ByteValue)", connection1);
                    command.Parameters.Add("@ByteValue", SqlDbType.Binary).Value = m_RegMin1;
                    connection1.Open();
      ...
      private void BtnVerify_Click(object sender, System.EventArgs e)
      {
         ...
         secu_level = (SGFPMSecurityLevel)comboBoxSecuLevel_V.SelectedIndex;
            command = new SqlCommand("SELECT [Fingerprint] FROM [User Fingerprint Table] WHERE [User ID] = 'pkellyi'", connection1);
            connection1.Open();
            Byte[] storedFPBytes = new Byte[400];
            storedFPBytes = (Byte[])command.ExecuteScalar();
            connection1.Close();

/*            for (int i = 0; i < storedFP.Length / 2; i++)
            {
                byte current = Convert.ToByte(storedFP.Substring(i * 2, 2));
                storedFPBytes[i] = current;
            }
*/
         ...
         iError = m_FPM.MatchTemplate(storedFPBytes, m_VrfMin, secu_level, ref matched1);
         if (iError == (Int32)SGFPMError.ERROR_NONE)
         {
